...
public class MainActivity extends Activity implements ServiceConnection {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        // Scan for music - 不确定的进度
        requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);

        // Layout - 加载布局
        setContentView(R.layout.library_browser);

        // Style the actionbar - 初始化 actionbar
        initActionBar();

        // Control Media volume  - 控制媒体音量
        setVolumeControlStream(AudioManager.STREAM_MUSIC);

        // Important!  - 初始化 ViewPager 和 PagerAdapter
        initPager();
    }
    ...
    public void initPager() {
        ...
        //只显示选项卡中设置首选项 Only show tabs that were set in preferences
        // 最近 Recently added tracks
        ...
        if (tabs_set.contains(getResources().getString(R.string.tab_songs)))
            mPagerAdapter.addFragment(new AudioPlayerFragmentList());
        // 播放列表 Playlists
    ...
    private void initActionBar() {
        ...
        // The ActionBar Title and UP ids are hidden. -  ActionBar标题和id是隐藏的。
